...
GR 곡선만으로 TVT가 하나로 결정되지는 않습니다. 다만 가능한 TVT path의 범위를 줄여줍니다. 최종 path는 geometry, 앞부분 calibration, formation position, smoothness와 서로 일관적인지를 봐야 합니다.
...
selector variant는 이 판단을 압축해서 표현한 이름입니다.
...
여기서 `S(X,Y)`는 공간상의 지층면, `b_w`는 시추공별 local offset입니다. 이 관계 때문에 `TVT`만 보는 것보다 `TVT + Z`를 보는 편이 더 안정적인 경우가 많습니다. 시추공이 기울어진 지층을 지나면 `Z`는 움직이지만, `TVT + Z`는 지층면에 상대적인 좌표처럼 움직입니다.
...
train horizontal file에 있는 formation column을 직접 쓰는 것은 위험합니다. test horizontal file에는 그 값이 없기 때문입니다. 따라서 EDA에서는 지질적으로 의미를 확인하되, 모델에는 fold 안에서 재현 가능한 surface estimator로 만든 값만 넣어야 합니다.

이 변환은 두 가지를 보장합니다. 첫째, test 시점에도 같은 변수를 만들 수 있습니다. 둘째, validation도 실제 추론 조건과 같은 추정 오차를 겪게 됩니다.
...
이 단계를 거치면 모든 plot이 무작정 모델 변수로 들어가는 일을 막을 수 있습니다. test 시점에 재현할 수 없는 값은 EDA에만 남겨야 합니다.
...
curve-level diagnostic, dense surface estimate, nearby-well spatial signal, representative well plot은 예측값을 그대로 가져다 쓰기 위한 결과물이 아니라, 신뢰도를 판단하는 근거로 사용합니다.
...
row random split은 이 질문에 답하지 못합니다. 같은 시추공 안의 행들은 강하게 이어져 있습니다. 바로 옆 행의 궤적, GR context, 지층 위치를 이미 본 상태라면, 모델은 처음 보는 well을 예측하는 것이 아니라 같은 well 안에서 interpolation을 하는 셈이 됩니다.
...
공개 점수와 비공개 점수의 해석은 이 위에 놓입니다. 어떤 feature는 GroupKFold에서 좋아도 same-well overlap에 의존하면 private에서는 약할 수 있습니다. 반대로 full test GR/trajectory를 쓰더라도 정답값을 보지 않는 방식이면 offline 환경에서도 안전하고 private에도 견고할 수 있습니다.
...
각 경로는 서로 다른 가정을 담고 있습니다. `hold`는 well이 같은 지층 위치에 머문다고 봅니다. flat tail에서는 강하지만 drift tail에서는 약합니다. formation path는 공간 구조를 믿습니다. GR이 비어 있어도 작동하지만 local offset에는 약할 수 있습니다. GR 대비 path는 typewell barcode가 충분한 정보를 준다고 봅니다. 반복되는 GR motif가 있으면 false match를 만들 수 있습니다. PF는 이 불확실성을 하나의 답으로 너무 빨리 접지 않고 순차적으로 가져갑니다.

particle filter의 state는 다음처럼 읽을 수 있습니다.
...
직접 `TVT`를 예측하면 well-level offset이나 row-position effect를 학습하기 쉽습니다. 반면 PF residual을 예측하면 질문이 좁아집니다.
...
fold 사이의 차이는 failure mode를 보여줍니다. 평균 RMSE만으로는 어떤 well group이 어려운지 알 수 없습니다. 다음 같은 요인을 따로 봐야 합니다.
