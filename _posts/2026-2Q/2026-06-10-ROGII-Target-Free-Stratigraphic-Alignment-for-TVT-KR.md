...
업데이트, 2026-07-03:  
이 글은 Series 1로서 ROGII를 **target-free stratigraphic alignment** 문제로 재해석하는 데 초점을 두었다. 이후 후속 노트와 공개 토론을 거치며 몇 가지 중요한 정정이 생겼다. 가장 낮은 GR matching cost가 반드시 올바른 TVT depth를 뜻하지는 않으며, GR은 정답 라벨이 아니라 competing minima를 가진 likelihood landscape로 읽어야 한다.

또한 datum recovery 이후 shape/slope가 중요한 modeling frontier로 남는 것은 맞지만, recoverable MSE mass 기준으로는 residual datum miss가 여전히 더 큰 몫을 차지한다. 이 후속 정리는 Series 2에 따로 정리했다: [ROGII Working Note (2편): Target-Free TVT Geosteering의 오차 해부](https://pilkwangkim.github.io/posts/ROGII-Working-Note-2-Target-Free-TVT-Geosteering-KR/).
...
이 문제에서 예측해야 하는 값은 수평정(horizontal well)의 숨겨진 후반 구간에 있는 `TVT`, 즉 true vertical thickness입니다. 각 테스트 시추공은 앞부분 일부 구간에 대해서만 `TVT_input`이 주어집니다. 그 뒤로 길게 이어지는 구간의 TVT는 `MD`, `X/Y/Z`, `GR` 같은 관측값만으로 복원해야 합니다.

겉으로 보면 한 컬럼을 맞히는 회귀 문제처럼 보이지만, 실제로는 조금 다릅니다. 각 행은 독립적인 표본이 아니라 시추공을 따라 지나가는 궤적상의 한 지점입니다. `MD`는 드릴이 실제로 지나간 경로 길이이고, `X/Y/Z`는 그 지점의 공간 좌표입니다. 수평정은 옆으로 수천 피트씩 이동해도 수직 위치는 상대적으로 천천히 변할 수 있습니다. 그래서 `TVT`는 단순히 “얼마나 깊은가”가 아니라, 시추공이 지층 안에서 어느 위치를 지나고 있는지를 나타내는 좌표에 가깝습니다.
...
한 행씩 따로 예측하는 문제라기보다는, 알려진 앞부분에서 시작해 숨겨진 후반 구간의 TVT 곡선을 이어 붙이는 문제입니다. 이 곡선은 시추공의 궤적, GR 패턴, 기준 typewell, 주변 지층 구조를 함께 보아야 합니다.
...
offline mode가 곧바로 데이터 누수라는 뜻은 아닙니다. Kaggle test 파일에는 전체 시추 궤적과 전체 GR sequence가 이미 들어 있습니다. 따라서 미래 행의 `GR`이나 `X/Y/Z`를 쓰는 것은 가능합니다. 문제는 그 정보가 숨겨진 TVT label을 거쳐 만들어진 값이면 안 된다는 점입니다.
...
held-out well에 해당 well의 true tail label, true formation top, target-derived summary를 넘겨주는 순간, 검증은 일반화 평가가 아니라 암기 테스트가 됩니다.
...
같은 well의 겹침 정보를 쓰는 경로는 일반적인 PF/beam selector와 성격이 다릅니다. 하나는 특정 overlap 구조를 이용하는 기하학적 지름길에 가깝고, 다른 하나는 보편적인 지층 추적기입니다. 이 둘을 명시적으로 나누어야 공개 점수가 무엇을 반영했는지 추적할 수 있습니다.
...
후속 working note에서 가장 크게 정정된 부분도 여기입니다. GR은 바코드처럼 작동할 수 있지만, **가장 낮은 GR cost를 주는 위치가 곧 정답 TVT depth는 아닙니다.** 반복되는 lithology motif 때문에 서로 다른 TVT depth가 비슷한 GR cost를 가질 수 있습니다. 따라서 GR 대비는 하나의 hard match가 아니라 competing minima와 confidence를 가진 likelihood landscape로 읽는 편이 더 안전합니다.
...
이 값은 시추공별 관측 noise 추정치가 됩니다. 앞부분 correlation이 약하거나 residual scale이 크면 typewell matching을 덜 믿어야 합니다.
...
여기서 `S(X,Y)`는 공간상의 지층면, `b_w`는 시추공별 local offset입니다. 이 관계 덕분에 `TVT`만 보는 것보다 `TVT + Z`를 보는 편이 더 안정적일 때가 많습니다. 시추공이 기울어진 지층을 지나면 `Z`는 변하지만, `TVT + Z`는 지층면 기준의 좌표처럼 비교적 안정적으로 움직입니다.
...
지층이 거의 평평하면 `S(X,Y)`가 천천히 변하므로 anchor가 강하게 유지됩니다. 반대로 지층이 기울어져 있으면 constant-TVT 경로는 설득력이 떨어집니다. 이때 앞부분 구간이 시추공별 offset인 `b_w`를 추정하는 데 쓰입니다.
...
curvature는 진행 방향 벡터의 변화를 통해 추정합니다. 이 값들은 target 자체가 아니라, 시추공이 공간에서 어떻게 움직이는지를 나타내는 값입니다. TVT가 얼마나 빠르게 변할 수 있는지에 대한 prior로 활용됩니다.
...
Series 2에서 이 부분을 더 분해해 보니, 해석을 조금 더 조심해야 했습니다. datum을 맞춘 뒤 shape/slope가 중요한 frontier로 남는 것은 맞지만, recoverable MSE를 기준으로 보면 residual datum miss가 더 큰 덩어리였습니다. 재계산한 split에서는 residual datum miss가 약 `70.3%`, pure shape가 약 `29.7%`를 차지했습니다. 즉 “이제 shape만 남았다”가 아니라, **shape를 모델링하되 남은 datum miss를 계속 방어해야 한다**는 쪽이 더 정확합니다.
...
선택한 strict feature set은 `calibrated_typewell_alignment`입니다.
...
strict feature set은 실시간 drilling에 가까운 보수적 설정을 확인하기 위한 묶음입니다. offline feature set은 Kaggle test 파일에 이미 제공된 전체 covariate path를 활용하되, 숨겨진 TVT 정답값은 보지 않습니다.
...
행 단위 random split은 이 질문에 답이 되지 않습니다. 같은 시추공 안의 행들은 서로 강하게 연결되어 있습니다. 이미 인접한 행의 궤적, GR context, 지층 위치를 본 상태라면, 모델은 처음 보는 well을 예측하는 게 아니라 같은 well 안에서 interpolation을 하는 셈이 됩니다.
...
| Public same-well enabled submission | public leaderboard에서 관측 가능한 same-well overlap이 얼마나 도움이 되는가? |
...
| formation path | `X/Y/Z`를 안전하게 추정한 formation surface로 TVT에 연결합니다. |
| same-well physical | train/test에 같은 well ID가 있을 때 geometry 겹침을 이용합니다. |

각 경로는 서로 다른 가정을 담고 있습니다. `hold`는 well이 같은 지층 위치에 머문다고 가정합니다. flat tail에서는 강하지만, drift tail에서는 약합니다. formation path는 공간 구조를 신뢰합니다. GR이 없어도 작동하지만, local offset에는 약할 수 있습니다. GR 대비 path는 typewell barcode가 충분한 정보를 준다고 봅니다. 반복되는 GR motif가 있으면 false match가 생길 수 있습니다. PF는 이 불확실성을 너무 빨리 하나의 답으로 접지 않고 순차적으로 끌고 갑니다.
...
DTW는 TVT label이 아니라 GR sequence를 맞추므로 정답값을 보지 않는 방법입니다. 위험은 정답 유출이 아니라, noisy/missing GR을 과신하는 데 있습니다.
...
tree model은 물리적 경로를 대체하지 않습니다. PF가 체계적으로 높거나 낮게 빗나가는 경우를 보정합니다.
...
row order가 틀리면 지질적으로 그럴듯한 곡선도 완전히 잘못된 제출이 되고 맙니다. 각 `id`는 특정 well의 특정 row index에 대응하기 때문입니다.
...
same-well contact로 얻은 높은 공개 점수가 곧바로 지질 모델의 강함을 증명하지는 않습니다. 정답값을 보지 않는 PF/beam/formation feature의 GroupKFold score가 private 일반화에는 더 많은 정보를 줄 수 있습니다.
...
이 문제는 평범한 row-wise regression이 아니라 **지층 경로 복원 문제**입니다.
