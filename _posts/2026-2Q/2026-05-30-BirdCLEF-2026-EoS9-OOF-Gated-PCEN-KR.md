---
title: "BirdCLEF 2026: EoS9 + OOF-Gated PCEN -KR"
...
# BirdCLEF 2026: EoS9 + OOF-Gated PCEN -KR
...
여기서 \(R(\cdot)\)는 class별 percentile rank입니다. 확률값 자체가 같은 calibration을 가진다고 보기 어렵기 때문에, raw probability보다 rank를 섞는 편이 더 안정적입니다. 즉, 질문을 이렇게 바꾸는 것입니다.
...
| Anchor 구성 | 역할 |
|---|---|
| **yukiZ Perch / ProtoSSM / ResidualSSM** | Perch 기반 pretrained path에서 오는 낮은 비중의 다양성입니다. |
...
이렇게 하면 점수에 영향을 주는 knob이 상단에 드러납니다. 또 가중치 합이 1인지 assert로 확인하기 때문에, 실험을 반복하다가 조용히 가중치 조건이 깨지는 일을 막을 수 있습니다.

## **3. Taxonomy Smoothing**

BirdCLEF label은 현실에서 서로 독립적이지 않습니다. 같은 genus에 속한 종, 같은 class에 속한 종, 비슷한 habitat와 acoustic confusion을 공유하는 종이 있습니다. Anchor가 이미 강한 class별 순서를 만들었다고 해도, 가까운 분류군끼리는 약간의 정보를 나누는 편이 도움이 될 수 있습니다.
...
핵심은 sidecar가 anchor를 대체하지 않는다는 점입니다. Sidecar는 anchor가 만든 순위를 조금 움직일 수 있을 뿐입니다.
...
PCEN sidecar는 custom log-mel / PCEN ConvNeXt path입니다. 활성 variant는 `exp002b_5s`이고, 주요 설정은 다음입니다.
...
최종 writer는 점수만큼 중요합니다. Kaggle 제출은 모델이 약해서가 아니라 `submission.csv`의 schema 문제가 조용히 섞여서 실패하는 경우가 많습니다.
...
이것은 단순한 방어적 코딩이 아닙니다. Sidecar 실험을 안전하게 만드는 핵심 장치입니다. BirdNET, PCEN, taxonomy smoothing, branch alignment 중 하나라도 schema를 깨면, anchor를 유지하거나 명확히 실패하게 만듭니다.

## **9. Output을 읽는 법**

긴 inference log보다 중요한 것은 compact summary입니다.
...
Public dry run에서의 핵심 흐름은 다음입니다.
