...
What's next follows directly. Rebuild the frontier engine in a clean, own form — gated collapse plus replay-safe sizing — and replace `FILL_BUDGET_FRAC`, the indirect proxy, with the direct replay-cost measurement, which removes the cliff-guessing entirely. Generalise the selector: rank templates by *measured raw per second*, $(16k + 2)/t$ for $k$ posts observed, which reduces to cheapest-per-fire in the single-post regime and, if any framing ever does chain, prefers it automatically — one rule that is optimal whether or not the raw ever lifts. Then probe the raw wall head-on with two engines pointed in different directions: one that maximises firing posts per second with the standard collapse, and one that adds templates priming a repeated `http.post` across the eight replay hops — the score gap between them measures whether the models can be pushed past a single post at all. The scorer gives the rules; the score is decided by four quantities — the GPU drawn, the replay cost paid, the reasoning the model can be made to skip, and the posts a single candidate can be made to fire — and those are the things to measure and control from here.
