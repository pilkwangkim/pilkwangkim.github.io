...
익숙한 Kaggle 예측 대회에서는 입력을 받아 예측값을 내는 모델을 만든다. 집의 여러 특성으로 가격을 맞히는 문제가 한 예다. 이번에는 입력으로 **프로그램의 오류를 설명한 글과 그 프로그램의 소스 코드**가 주어진다. 시스템이 오류의 원인을 찾아 코드를 고쳐야 한다. 어디를 고칠지부터 알아내야 한다는 점이 핵심이다.

[Google — Gemma 4 Developer Agent Competition][competition]에서는 모든 에이전트가 정해진 Gemma base model을 사용해야 한다. 참가자는 이 모델이 문제를 풀어 가는 방식을 설계한다. 어떤 코드부터 읽을지, 어디를 고칠지, 수정 결과를 어떻게 확인할지 정하고, 실패했을 때 다시 시도하는 방법까지 마련하는 것이다. Kaggle은 제출된 에이전트에 숨겨진 이슈를 맡기고, 만들어 낸 패치를 평가한다. **공개된 연습 문제의 수정안을 제출하는 대회가 아니라, 새로운 문제를 해결할 에이전트를 제출하는 대회다.**
...
코드 모델은 잘못된 한 줄을 프롬프트에 넣어 주면 수정안을 제시할 수 있다. 개발 에이전트는 그 한 줄을 찾는 일부터 맡는다. 고친 코드가 실제로 도움이 되었는지도 확인해야 한다. 예를 들어 “limit을 0으로 지정했는데 항목이 열 개 나온다”는 이슈를 받으면, 0이 어디서 10으로 바뀌었는지 찾아야 한다. 그 동작을 고치면서도 limit을 생략했을 때의 기본값은 유지해야 한다. 1절에서는 이 작은 예를 끝까지 따라가 본다.

그래서 코딩 에이전트는 모델을 여러 번 호출한다. 검색으로 파일명을 알아내고, 파일을 읽어 함수를 찾고, 테스트 결과를 보고 처음 생각한 원인이 틀렸음을 깨달을 수 있다. 다음에 할 일은 직전 행동으로 무엇을 알아냈는지에 따라 달라진다. 필요한 정보를 모두 한꺼번에 주는 문제가 아니라, 조사하면서 필요한 정보를 찾아가는 문제다.

### 우리가 제출하는 것과 에이전트가 만드는 것

**저장소(repository)**에는 소스 코드, 테스트, 설정, 문서와 변경 이력이 들어 있다. **이슈(issue)**는 고치거나 추가할 동작을 설명한다. **패치(patch)**는 파일을 어떻게 바꿨는지 기록한 것이며, Git이 그 변경을 텍스트로 나타낸 것을 **diff**라고 부른다.

우리가 제출하는 것은 설정과 지침, 필요한 경우 학습한 adapter를 묶은 **에이전트 패키지**다. 이를 실행하면 **이슈마다 패치**가 만들어진다. 별도의 평가기는 원래 저장소를 새로 준비해 패치를 적용하고 **해결 여부**를 판정한다. 패키지가 로드되는 것, 내용이 있는 diff가 나오는 것, 검증을 통과하는 것은 각각 다른 단계다. 점수는 마지막 단계까지 통과한 문제를 기준으로 계산한다.
...
*그림 1. 문제 하나를 푸는 동안 에이전트는 저장소를 읽고, 고치고, 검사하는 과정을 반복한다. 참가자는 공개 개발 데이터에서 얻은 실패 기록을 읽고, 에이전트를 고쳐 다시 실험한다. Kaggle은 제출된 설계를 숨겨진 문제에 적용하며, 그 문제의 정답을 에이전트 입력에 넣어 주지 않는다. 번호가 붙은 세 칸은 각 주체가 맡은 일을 구분한 것으로, 성능 측정값이 아니다.*

### 모델이 같아도 달라질 수 있는 것

**모델(model)**은 현재 입력을 바탕으로 응답을 만든다. **에이전트(agent)**는 여기에 지침과 도구, 지금까지 얻은 정보, 작업을 이어 가는 실행 루프를 더한 시스템이다. **도구(tool)**는 파일 읽기나 명령 실행을 실제로 수행한다. 주최 측이 제공하는 **harness**는 이들을 연결하고, 저장소 환경을 준비하며, 실행 제한을 적용하고 최종 패치를 검사한다. 디스크에 파일이 있다고 모델이 그 내용을 저절로 아는 것은 아니다. 도구를 통해 읽어 와야 한다.

기본 모델이 같다고 에이전트의 행동까지 같아지는 것은 아니다. 어떤 지침을 줄지, 허용된 도구 가운데 무엇을 연결할지, 다른 에이전트에 무슨 일을 맡길지, 실행 한도를 어떻게 정할지 바꿔 볼 수 있다. 예를 들어 **localization**은 이슈와 관련된 파일과 심볼을 좁혀 가는 작업이다. 검색 방법을 개선하면 모델이 엉뚱한 파일 대신 문제와 관련된 함수를 읽게 할 수 있다. 편집 절차를 개선하면 맞는 수정안을 생각해 놓고도 도구 호출에 실패해 파일을 바꾸지 못하는 일을 줄일 수 있다. 테스트를 잘 고르면 수정 방향이 잘못됐다는 사실을 일찍 알아내 다시 시도할 시간을 남길 수 있다. 모두 새 가중치를 학습하지 않고도 해결률에 영향을 줄 수 있는 변화다.

공식 평가 기준은 주최 측이 정한다. 우리가 준비할 로컬 평가 환경은 그 기준에 맞춰 패치를 실행하고, 실패한 이유를 확인할 수 있어야 한다. 평가 환경을 개선한다는 것은 이 재현과 진단을 더 정확하게 한다는 뜻이다. Kaggle의 채점 기준을 바꾸는 것은 아니다. 에이전트가 디버깅 중에 실행하는 검사도 다음 행동을 정하기 위한 근거다. 점수를 정하는 것은 나중에 별도로 수행하는 평가이며, 에이전트가 숨겨진 검증 테스트를 미리 실행해 가며 답을 고를 수는 없다.

추가 학습을 시도할 때는 **LoRA adapter**를 쓸 수 있다. 지정된 base model에 학습으로 얻은 가중치 변화량을 더하는 방식이다. 어떤 행동을 학습시켜야 할지 파악한 뒤 시도할 수 있는 방법이다. 에이전트를 늘리거나, 검색을 더 하거나, 학습을 추가하면 자원도 더 든다. 같은 평가 조건과 제한 시간 안에서 더 많은 문제를 고치는 데 도움이 되는지 확인해야 한다.

### 첫 실험에서 확인할 것

처음에는 에이전트가 이슈 하나를 받아 패치를 내기까지의 과정을 끝까지 따라가 보자. 어떤 판단을 했고, 왜 성공하거나 실패했는지 설명할 수 있으면 된다. 다음 세 질문을 차례로 확인하면 된다.

1. **로컬 검사 결과를 믿을 수 있는가?** 개발 과제 하나를 골라, 수정 전 코드와 제공된 기준 수정안인 reference patch를 적용한 코드가 각각 예상대로 실패하고 통과하는지 확인한다. 환경 문제로 실패했다면 프롬프트부터 고칠 일이 아니다.
2. **단순한 에이전트는 실제로 무엇을 하는가?** 검색, 파일 읽기, 편집, 테스트 결과, 최종 패치와 소요 시간을 남긴다. 이 행동과 관찰의 기록을 **trajectory**라고 한다. 편집 호출에 실패한 경우와 원인을 잘못 짚은 경우는 모두 0점이어도 고칠 부분이 다르다.
3. **실패 원인을 보고 고친 부분이 다른 문제에서도 도움이 되는가?** 바꾼 에이전트와 baseline을 같은 조건에서 비교하고, 개발에 쓰지 않은 문제도 포함해 확인한다. 그 뒤 남은 실패를 보고 학습이나 역할 분리가 필요한지 판단한다.

우리가 반복할 실험도 이 순서를 따른다. 에이전트를 실행해 실패 원인을 찾고, 그 원인을 줄이도록 한 가지를 바꾼 뒤, 새로운 문제에서도 효과가 있는지 비교한다. 1–4절에서는 버그 하나가 시스템을 거쳐 점수가 되는 과정을 살펴본다. 5–8절에서는 관련 연구와 데이터, 제약 조건을 설명한다. 9–12절에서는 baseline을 직접 만들고 비교 실험을 준비하며, 13절에서 학습으로 개선하는 방법을 다룬다.

간단한 Python 코드를 읽을 수 있으면 따라올 수 있도록 썼다. 프레임워크와 학습 관련 용어는 필요할 때 설명한다. 실행 예제의 설정은 **아직 성능을 측정하지 않은 실습용 예시**로, 프로젝트에서 실험 중인 후보와는 별개다. 대회 세부 사항은 **2026년 9월 25일**에 확인했다. 긴 코드는 접어 두었으므로 직접 구현할 때 펼쳐 읽으면 된다.
...
에이전트가 `limit`을 검색했더니 수십 개의 결과가 나왔다고 하자. 처음 눈에 들어온 `routes.py`에 0을 처리하는 예외를 넣을 수도 있다. 그러면 웹 endpoint는 고쳐져도 명령줄 entry point에는 오류가 남을 수 있다. 두 경로가 결국 `limits.py`를 함께 호출하기 때문이다. 반대로 공통 함수에서 필요 이상으로 동작을 바꾸면 보고서에 언급되지 않은 빈 문자열의 처리까지 바뀔 수 있다.
...
세 입력은 각각 다른 것을 확인한다. 0은 버그를 재현하고, `None`은 수정 후에도 기본값이 적용되는지 확인한다. 5는 일반적인 입력도 여전히 제대로 처리되는지 확인한다. 다른 입력 타입도 허용하는 저장소라면 그때의 동작은 별도로 살펴야 한다. 이 세 예만 보고 다른 입력이 어떻게 처리되어야 하는지까지 에이전트가 정해서는 안 된다.
...
모델이 다음에 할 일을 제안하면 에이전트의 실행 루프가 허용된 도구를 호출하고, 그 실행 결과를 모델에 돌려준다. harness는 작업 환경과 실행 시간을 관리하고 패치의 검증 결과를 판정한다. 모델에 어떤 정보를 전달하고, 요청한 명령의 결과를 어떻게 돌려주느냐에 따라서도 최종 결과가 달라질 수 있다.
...
언어 모델이 학습한 내용은 parameter라는 수치에 반영되어 있다. 보통 **가중치(weights)**라고 부르는 값이다. **추론(inference)**은 이 가중치와 현재 입력으로 응답을 만들거나 다음에 할 일을 정하는 과정이다. 모델은 텍스트를 토큰 단위로 처리하고 출력한다. 단어의 일부, 문장부호, 코드 식별자의 일부가 토큰이 될 수 있다.
...
이 차이를 알면 개선 방법도 나누어 볼 수 있다. 프롬프트나 도구를 바꿔서 같은 모델이 더 유용한 정보를 보게 할 수 있다. 추가 학습을 통해 여러 상황에서 더 적절한 선택을 하도록 가르칠 수도 있다. 지금 무엇 때문에 실패하는지 알아야 어느 쪽을 시도할지 정할 수 있다. 뒤에서 다룰 **LoRA adapter**는 학습한 가중치 변화량을 작은 형태로 저장한 것이다. 저장소 파일이나 문제별 정답 패치를 대신 담는 파일이 아니다.
...
개발팀이 모델을 직접 실행하면 무엇이 좋을까. 소스 코드를 처리할 장비를 직접 정할 수 있고, 필요할 때 같은 모델 버전으로 다시 실행할 수 있다. 네트워크가 불안정한 곳에서 쓰거나 특정 업무에 맞게 시스템을 바꾸려는 경우에도 유용할 수 있다. 로컬 에이전트에 관심을 갖는 이유다. 다만 직접 실행한다고 개인정보 보호나 저렴한 비용, 빠른 속도가 자동으로 보장되지는 않는다. 어떤 도구와 로그를 쓰느냐에 따라 데이터가 이동하는 경로가 달라지고, 실행 속도는 하드웨어와 작업량의 영향을 받는다.
...
운영진이 이 대회로 발전시키려는 것은 코드를 탐색하고 수정안을 만드는 공개 에이전트다. 아키텍처를 설계하고 변경을 검토하는 일은 사람이 계속 맡는다. 대회는 fine-tuning과 강화학습을 장려하지만 **제출 형식에서 adapter는 선택 사항**이다. 따라서 추가 학습 없이 시작해도 된다. 먼저 지정된 모델이 어느 정도까지 해내는지 알아야 나중에 좋아진 결과가 학습 덕분인지 판단할 수 있다. [대회 개요와 모델 규칙][competition]

기본 모델이 같으면 비교할 대상이 분명해진다. 모델에 보여 주는 정보, 일을 진행하는 순서, 규칙 안에서 적용할 수 있는 추가 학습을 바꿔 보고, 같은 연산 자원과 제한 시간 안에서 더 많은 이슈를 해결하는지 비교할 수 있다. 물론 팀마다 학습에 쓸 수 있는 자원은 다르다. 모델을 하나로 정했다고 실험 여건까지 같아지는 것은 아니다. 다만 제출한 에이전트를 어떤 모델과 실행 환경에서 비교하는지는 분명해진다.
...
METR의 [2025년 7월 연구][metr-2025]는 이 차이를 생각해 볼 사례다. 숙련된 오픈소스 개발자 16명과 작업 246개를 대상으로 한 무작위 실험에서, 2025년 초의 AI 도구를 사용할 수 있었던 조건에서 작업 완료 시간이 19% 늘었다. 특정 개발자 집단이 그 시기의 도구로 특정 방식의 작업을 했을 때 나온 결과다. 이 수치가 앞으로도 모든 AI 지원의 효과를 설명하는 것은 아니다.

같은 연구팀이 낸 [2026년 2월 후속 설명][metr-2026]도 함께 봐야 한다. 새 실험에서는 AI 사용을 금지하는 조건에 개발자나 작업이 포함되지 않는 등 선택 편향이 생겼다고 밝혔다. 저자들도 이 새 추정치만으로는 당시 AI 도구가 생산성에 미친 영향을 신뢰성 있게 판단하기 어렵다고 설명했다. 어느 연구든 제목만 보고 보편적인 결론을 내리면, 누구를 대상으로 무엇을 측정했는지 놓치게 된다.
...
에이전트는 `/workspace`에 준비된 저장소에서 작업한다. 코드를 살펴보고 diff를 만드는 데 필요한 Git 상태는 남아 있지만, 이후 정답 수정이 들어간 커밋은 제거되어 있다. 공개 개발 데이터에는 reference patch인 `patch`와 검증에 쓰는 `test_patch`도 들어 있다. 학습과 로컬 검증에 쓰라고 제공한 자료다. **holdout을 평가할 때 에이전트에게 정답으로 건네는 자료가 아니다.** 비공개 평가에서도 에이전트는 reference patch와 검증 테스트를 볼 수 없다. [데이터셋 명세][data]
...
위 명령에 지정한 이름의 기준 커밋이 없으면 `git diff --binary HEAD`로 다시 시도한다. 첫 번째 명령은 커밋을 만들지 않는다. 새 파일도 diff에 나타나게 할 뿐이다. 그래서 저장소에 남겨 둔 임시 재현 스크립트가 의도치 않게 제출 패치에 들어갈 수 있다. [공개 패치 추출 소스][wheelhouse]

두 번째 환경에서는 harness가 새로 준비한 기준 저장소에 패치를 적용한다. 보호 대상 테스트와 실행 설정 파일의 복원을 시도하고, 평가기의 `test_patch`를 적용한 뒤 지정된 테스트를 실행한다. 공개된 검증 코드는 test patch에서 pytest 실행 대상을 찾는다. 데이터 설명에는 reference patch를 선별할 때 전체 테스트를 확인한다는 내용도 있지만, 자료를 선별하는 절차와 개별 과제를 채점하는 절차는 다르다. 공개 verifier는 실제 테스트 결과가 기록되었는지, 결과 형식이 올바른지도 확인한다. 프로세스가 오류 없이 끝났다는 사실만으로 통과 처리하지는 않는다. [harness 안내][data] · [공개 검증 소스][wheelhouse]
...
반면 **평가기의 검증 테스트**는 최종 패치가 채점 기준을 통과하는지 확인한다. harness가 별도의 새 환경에서 실행한다. 비공개 문제를 푸는 에이전트가 패치 후보를 고를 때마다 이 판정을 미리 받아 볼 수는 없다. 공개 개발 데이터에서 연습용으로 고른 문제라면 제공된 수정안과 검증 자료를 읽으며 공부할 수 있다. 하지만 평가용으로 남겨 둔 문제의 성능을 측정할 때는 그 자료를 에이전트 입력에 넣지 않는다. 에이전트가 실행한 검사에 통과했더라도, 실제 점수는 나중의 평가기 검증 결과로 정해진다.

다만 복원이 항상 성공한다고 가정해서는 안 된다. 확인한 `swegemma 0.2.7`은 복원할 경로들을 `git checkout HEAD -- ...`에 한꺼번에 넘긴다. 기준 커밋에 없는 경로 때문에 이 명령이 실패해도 검증은 계속 진행된다. Git 2.54로 재현했을 때는 checkout이 실패하면서 기존 테스트 파일의 수정이 그대로 남았다. 별도로 실행하는 clean은 목록에 있는 untracked 파일을 지운다. 따라서 테스트 수정이 모두 제거된다고 설명할 수는 없다. 이는 확인한 로컬 소스의 동작이며, 숨겨진 채점기에서도 똑같이 동작한다는 증거는 아니다.

여기서 두 가지를 지켜야 한다. 첫째, 에이전트가 실제 구현을 고치도록 해야 한다. 테스트의 기대값을 바꿨다는 사실은 보고된 버그를 해결했다는 근거가 되지 않는다. 둘째, 추출한 패치만 적용해도 성공을 재현할 수 있어야 한다. 메모리 안에서만 바꾼 값이나 작업 중 따로 설치한 패키지, 기록하지 않은 환경 설정에 의존한다면 다른 환경에서 그대로 쓸 수 있는 수정이라고 보기 어렵다.
...
Public 리더보드에는 비공개 평가 데이터의 대략 절반을 쓰고, 나머지 절반으로 최종 순위를 정한다. Public 점수만 보며 거듭 조정한 설정이 최종 평가에서도 유리하리라는 보장은 없다. 나는 먼저 공개 개발 과제에서 무엇이 왜 좋아졌는지 확인하려 한다. 그다음 횟수가 제한된 리더보드 제출로 다른 문제에서도 같은 개선이 나타나는지 살펴볼 생각이다.
...
세로줄은 ‘앞의 조건이 성립했을 때’를 뜻한다. 두 번째 항부터는 **이전 단계까지 성공한 시도 가운데** 다음 단계도 성공할 확률을 나타낸다. 확률의 연쇄 법칙이므로 단계별 실패가 서로 독립이라고 가정하지 않는다. 여기서는 실패가 어디에서 생기는지 설명하려고 과정을 네 단계로 나눴다. Kaggle이 단계마다 별도 점수를 준다는 뜻은 아니다.
...
지금까지 에이전트가 해야 할 일과 성공을 판단하는 기준을 살펴봤다. 다음 연구들은 그 과정의 서로 다른 부분을 다룬다. 생성한 코드가 맞는지, 도구를 실행한 결과를 다음 판단에 어떻게 쓸지, 저장소에서 필요한 코드를 어떻게 찾을지, 모델이 컴퓨터를 어떻게 다룰지에 관한 연구다. 각 연구가 우리 에이전트의 어떤 부분을 개선하는 데 도움이 될지 생각하며 읽어 보자. 연구마다 모델과 데이터, 사용한 자원이 다르므로 대표 점수를 이 대회의 성능 순위처럼 비교할 수는 없다.
...
이 논문은 답을 여러 번 sampling하는 경우도 다룬다. 여러 후보 중 하나가 맞는 것과 첫 번째 답부터 맞는 것은 서로 다른 결과다. 앞서 살펴본 재시도와 후보 선택에서도 이 차이가 중요하다. 다만 함수 완성 문제에서는 코드를 작성할 위치가 어느 정도 정해져 있다. 첫머리의 버그 보고서는 그렇지 않았다. 수정할 함수를 찾는 일부터 필요했다.
...
2024년에 처음 공개되고 ICLR 2025에 채택된 OpenHands 논문은 에이전트와 runtime, 도구 실행과 결과 처리, skill, 다른 에이전트에 일을 맡기는 기능을 함께 갖춘 플랫폼을 설명한다. 여기서 눈여겨볼 점은 어떤 명령을 실행할지 정하는 일과 그 명령을 실제로 실행하는 일이 나뉘어 있다는 것이다. [논문][openhands-paper] · [프로젝트 GitHub][openhands-code]
...
FastAPI와 Rich가 129개 중 115개, 약 89%를 차지한다. 이 두 프로젝트에서 좋아진 성능이 전체 평균을 크게 끌어올릴 수 있다는 뜻이다. 반면 Requests나 HTTPX에서도 잘 작동하는지는 문제 수가 적어 판단하기 어렵다. 저장소가 네 개라는 사실만으로 네 프로젝트를 고르게 검증했다고 볼 수는 없다.
...
일반적인 Kaggle 대회에 익숙하다면 데이터 형식부터 다르게 느껴질 것이다. 여기서 데이터 한 행에는 소프트웨어 이슈 하나를 풀고 평가하는 데 필요한 정보가 담겨 있다. 입력 특성과 목표값만 있으면 되는 것이 아니다. 해당 시점의 프로그램과 의존성을 복원하고, 수정이 성공했는지 가려낼 테스트까지 준비해야 한다. 과제가 129개뿐인데도 데이터 크기가 수십 GB에 이르는 이유다.
...
나라면 기존 파일·폼 테스트와, endpoint에 선언된 매개변수를 실제 요청 처리로 연결하는 코드를 먼저 살펴보겠다. 입력은 그대로 두고 매개변수 순서만 바꾸면 작은 재현 예제를 만들 수 있다. 어느 계층을 고칠지는 반환된 검증 오류의 세부 내용을 읽은 뒤 정한다. 이렇게 하면 reference patch로 정답 위치를 알아내지 않고도, 드러난 증상에서 출발해 조사할 곳을 좁혀 갈 수 있다.
...
`.jsonl`은 한 줄에 JSON 객체 하나를 담는 파일 형식이다. 각 과제의 데이터에는 용도가 다른 정보가 함께 들어 있다. 처음부터 용도를 구분해 두면 평가할 때 혼동을 줄일 수 있다.
...
개발 과제와 숨겨진 과제의 출처도 다르다. 개발 과제는 공개 프로젝트 네 개에서 가져왔지만, 숨겨진 과제는 **비공개 저장소**에서 선별했다. 따라서 익숙한 프로젝트의 디렉터리 구조에 기대어 프롬프트를 만들기보다, 낯선 저장소에서도 필요한 코드를 찾아낼 방법을 익혀야 한다.

여기서 **일반화(generalization)**는 환경이 달라져도 같은 방법이 계속 도움이 된다는 뜻이다. “공통 함수를 고치기 전에 호출 경로를 따라 입력이 어떻게 전달되는지 확인한다”는 절차는 다른 프로젝트에서도 쓸 수 있다. 반면 “이런 문제의 답은 항상 이 파일에 있다”는 판단은 공개 저장소에 익숙해서 가능한 것일 수 있다. 벤치마크 점수가 올랐다면 무엇을 배운 결과인지도 살펴봐야 한다. 일반적인 조사 절차가 좋아진 것인지, 익숙한 파일을 더 잘 찾아낸 것인지에 따라 새 문제에서 기대할 수 있는 성능이 달라진다.
...
앞의 제한값 예제에서는 그래프를 통해 `parse_options`와 `normalize_limit`의 연결을 확인하고, 같은 함수를 호출하는 다른 코드도 찾을 수 있다. 공통 코드를 고쳤을 때 어디까지 영향을 줄지 살펴보는 데 도움이 된다. **embedding**은 인덱싱된 코드 항목을 숫자 벡터로 나타낸 것이다. 유사도 검색은 이 벡터들을 비교해 서로 가까운 항목을 찾는다. 다만 비슷한 코드라고 해서 버그의 원인인 것은 아니다. 다음에 어디를 살펴볼지 정하는 데 쓸 수 있는 단서다.

예를 들어 텍스트 검색으로 인덱스에 있는 함수 하나를 찾았다면, 그 함수에 저장된 벡터 $\mathbf{q}$와 다른 심볼의 벡터 $\mathbf{z}_j$를 비교할 수 있다. 두 벡터가 모두 영벡터가 아닐 때 cosine similarity는 다음과 같다.
...
값이 클수록 두 벡터의 방향이 비슷하다는 뜻이다. 그 심볼에 버그가 있을 확률은 아니다. 제공된 도구는 이미 인덱싱된 심볼에서 출발한다. 임의의 이슈 문장으로 검색하려면 저장된 벡터와 호환되는 query embedding 방법이 따로 필요하다. 유사도 수식만 있다고 그 과정까지 구현되는 것은 아니다.

실제로는 이슈에 나온 이름을 텍스트로 검색하고, 찾은 소스를 읽은 뒤, 구체적인 가설에 따라 유사 심볼이나 그래프의 이웃을 살펴보는 식으로 조합할 수 있다. 조회가 안 되면 텍스트 검색으로 돌아간다. 여러 검색 방법을 함께 쓰더라도, 처음부터 점수에 가중치를 붙여 합칠 필요는 없다. 검색을 한 단계 더 했을 때 제한된 시간과 context 안에서 유용한 코드를 더 찾는지부터 비교하면 된다.
...
개발 데이터에는 확인이 필요한 문제도 보고되어 있다. 이 글을 작성하면서 살펴본 대회 파일 목록에는 `graphs/`에 0바이트 항목 129개, `embeddings/`에도 129개가 있었다. 한 [참가자의 조사][graph-discussion]에서는 과제 이름과 커밋 이름으로 된 hard link를 처리하는 방식 때문에 빈 복사본이 생겼다고 설명하며, async 함수가 누락된 문제도 보고한다. 이 글에서 실시간 파일 목록과 대조해 확인한 것은 0바이트 크기다. async 함수가 얼마나 누락됐는지는 해당 참가자가 조사한 내용이며, 이 글에서 직접 측정한 결과는 아니다. 운영진은 보고를 확인하고 조사 중이다.
...
위 사양의 출처는 [harness 안내서][data]다. 읽을 때는 로컬 라이브러리의 기본값과 Kaggle 채점기의 설정을 구분해야 한다. Kaggle 채점기는 라이브러리를 별도로 연동하므로, 로컬 기본값이 그대로 적용되지 않는 부분이 있다.
...
로컬 에이전트 세션의 시간은 작업 환경 준비가 끝난 뒤부터 잰다. 반면 Kaggle의 12시간 제한에는 준비 시간도 포함된다. 작업별 에이전트 timeout을 줄여도 환경 준비에 걸리는 시간까지 줄어들지는 않는다.
...
같은 시간 구간을 두 항목에 중복해서 더하지 않도록 주의한다. 검증 시간은 이 패치 생성 제한 시간에 포함되지 않는다. 검증 시간까지 왼쪽에 더하면, 패치 생성에 걸린 시간보다 넓은 범위를 계산하게 된다.
...
서버는 응답을 생성하기 전에 입력 토큰 수와 **요청한 최대 출력 토큰 수**를 합쳐 context 한도를 넘는지 확인한다.

$$
C_{\mathrm{prompt}}+M_{\mathrm{requested}}\leq32768.
$$

$C_{\mathrm{prompt}}$에는 지침, 이슈 설명, tool schema, 도구로 읽어 온 내용과 template overhead가 모두 들어간다. 예제처럼 출력 한도를 $M_{\mathrm{requested}}=8192$로 요청하면 입력에 쓸 수 있는 공간은 최대 24,576토큰이다. 실제로는 짧은 답을 낼 예정이더라도 이 요청 검사에는 출력 한도가 사용된다. 따라서 표시된 context 한도까지 소스 코드를 채우지 말고 여유를 남겨야 한다. [서버의 요청 검사][vllm-context]
...
먼저 직접 살펴볼 작업 몇 개를 고른다. 에이전트를 설계하고 연습하는 데 쓸 개발용 예제다. 답안을 읽었거나 그 문제에 맞춰 프롬프트를 여러 번 고쳤다면, 나중에 성공하더라도 이미 익숙한 문제를 푼 결과로 봐야 한다. 독립적으로 평가할 문제는 별도의 **holdout**으로 남겨 둔다. 평가할 에이전트를 정하기 전에는 그 문제의 답안이나 평가 결과를 보고 설계를 고치지 않는다는 뜻이다.
...
표의 결과는 각 대조군에서 나와야 할 결과다. 실제로는 선택한 공개 과제나 로컬 환경에 문제가 있어 다르게 나올 수 있다. reference patch를 적용했는데도 실패하면 기록을 남기고 원인부터 조사해야 한다. 원인을 확인한 뒤에야 그 작업의 결과로 모델 성능을 평가할 수 있다. 반대로 수정 전 저장소가 통과한다면, 나중에 에이전트가 통과하더라도 실제로 결함을 고친 것인지 다시 살펴야 한다. 대조군을 실행한다고 평가 자체의 문제가 해결되지는 않지만, 어디부터 조사할지는 알 수 있다.
...
여기까지 준비했다면 연습 이슈가 요구하는 동작과, 올바르게 고쳤는지 확인할 방법을 설명할 수 있어야 한다. 평가 환경에 문제가 생겼을 때 이를 모델의 실패와 구분할 수도 있어야 한다. 이런 준비 없이 모델 출력부터 많이 모으면, 나중에 무엇을 학습에 써도 되는지 판단하기 어렵다.
...
**baseline**은 이후에 만든 에이전트와 성능을 비교할 기준 버전이다. 처음에는 동작을 이해할 수 있고 같은 조건에서 다시 실행할 수 있어야 한다. 두 번째 에이전트와 그래프 도구, adapter를 한꺼번에 추가하면 결과가 달라져도 무엇이 도움이 되었는지 알기 어렵다. 아래의 작은 패키지에서 출발하면 변경 사항을 하나씩 나누어 실험할 수 있다.
...
배포된 로컬 검증기는 마지막 pytest 실행에도 같은 명령 제한 시간을 적용한다. 여기서는 최종 검사가 짧은 60초 제한 때문에 중단되지 않도록 300초로 설정한다. 이 값과 3분짜리 에이전트 세션 제한은 별개다. 에이전트가 실행하는 명령은 세션의 남은 시간 안에 끝내야 하지만, 새 환경에서 하는 검증은 세션이 끝난 뒤 시작된다. 선택한 작업에 300초가 충분한지는 수정 전 저장소와 reference patch를 적용한 저장소에서 예상한 결과가 나오는지 보고 판단한다. 검증기가 시간을 초과한 경우는 모델의 추론 실패와 구분해 기록해야 한다. [배포된 평가기와 검증 소스][wheelhouse]
...
압축 파일 목록에는 `agent/agent.yaml`이 아니라 루트의 `agent.yaml`이 보여야 한다. 파일의 타임스탬프를 고정했으므로, 같은 빌드 환경에서 같은 파일을 다시 압축하면 동일한 ZIP을 만들 수 있다. 실험마다 출력된 SHA-256 해시를 기록해 두면 나중에 어떤 압축 파일로 채점받았는지 확인할 수 있다.
...
이제 준비한 패키지에 문제 데이터와 실행 환경을 연결해 보자. 수정 전 저장소와 reference patch를 적용한 저장소를 각각 검증한 뒤 모델 서버를 시작하고 에이전트의 실행 기록을 얻는다. reference patch 검사 스크립트는 모델에게 문제를 풀게 하지 않고도 평가 환경을 확인하는 방법이다. 이후 GPU 실행에서는 에이전트가 그 환경을 제대로 사용할 수 있는지 살펴본다. 모델 가중치, 내려받은 문제, 대조군 검사 스크립트, 결과 로그는 개발용 자료이므로 제출할 에이전트 폴더 밖에 둔다.
...
이 API 예제는 배포 소스를 확인해 작성했다. 실행에는 호스트 의존 패키지가 필요하지만 모델을 호출하지는 않는다. schema 오류가 있으면 추론을 시작하기 전에 수정한다. 에이전트와 도구가 실제로 등록되고 실행되는지는 이후 평가기를 돌리면서 확인한다.
...
첫 실행에서는 적은 수의 문제로 전체 과정이 끝까지 동작하는지 확인한다. 에이전트를 불러올 수 있는지, 지정 모델이 실행 가능한 도구 호출을 반환하는지, 저장소 환경이 초기화되는지, 패치를 추출하고 검증 결과를 얻을 수 있는지 차례로 살핀다. 문제를 해결하지 못했더라도 모든 단계를 실행하고 실패 원인을 추적할 수 있었다면, 실행 환경을 점검하는 smoke test로서는 의미가 있다.
...
지정한 모델이 요청을 처리하고, 문제의 실행 환경이 준비되고, 패치 추출과 검증까지 끝났는지 확인한다. 결과와 소요 시간도 남겼다면 smoke test를 마치고 두 에이전트를 비교할 준비가 된 것이다. 이때 해결한 문제 수는 0이어도 된다. 이 단계에서는 문제를 얼마나 잘 푸는지에 앞서 전체 과정을 실행할 수 있는지부터 확인하기 때문이다. 중간에 실행되지 않은 단계가 있다면, 프롬프트를 바꿔 성능을 비교하기 전에 그 연결부터 고쳐야 한다.
...
문제 하나를 처음부터 끝까지 평가할 수 있게 되었다면, 이제 비교에 쓸 문제 목록을 고정한다. baseline과 설정을 바꾼 새 버전이 같은 문제를 같은 환경과 제한 아래에서 풀게 한다. 같은 문제 $M$개를 풀게 했을 때 두 버전의 해결률 차이는 다음과 같다.
...
앞 절의 편집 실패 사례를 다시 보자. 모델은 `normalize_limit`을 찾았지만, 바꾸려는 문자열이 실제 파일과 달랐다. 도구는 실패를 알렸는데 에이전트는 수정이 끝난 것처럼 다음 작업을 진행했다. 이 예에서 실행 기록만 보고 알 수 있는 사실은 여기까지다. 곧바로 “코딩 지식이 부족하다”라고 판단하기보다, 편집 실패 뒤에 해야 할 일을 분명히 알려 주면 어떨지 실험해 볼 수 있다. 저장되지 않은 코드를 전제로 다음 작업을 진행하는 일이 줄어들 수 있다는 가설이다.
...
이 변경으로 **앞서 든 가상의 12개 문제 결과**가 나왔다고 하자. 새로 푼 문제는 세 개이고, 오히려 놓친 문제는 두 개다. 성공한 세 문제에서는 편집 오류를 바로잡았지만, 실패한 두 문제에서는 파일을 계속 다시 읽다가 시간이 끝났다면 어떨까. 이 지시가 편집 오류를 바로잡는 데 도움이 되더라도, 재시도에 너무 오래 매달리게 할 수 있다는 뜻이다. 다음에는 재시도 횟수를 제한한 버전을 비교해 볼 수 있다.
...
이슈가 달라도 같은 저장소 스냅샷을 사용할 수 있다. 내려받은 문제 파일에는 `(repo, base_commit)` 조합이 127개 있다. `rich_3882`와 `rich_3894`는 같은 그룹이고, `requests_6589`와 `requests_6629`도 같은 그룹이다. 행을 무작위로 나누면 밀접하게 연결된 코드가 개발용과 평가용에 동시에 들어갈 수 있다. 같은 스냅샷을 쓰는 문제는 개발용이나 평가용 중 한쪽에 함께 넣는다. 그 밖에도 내용이 비슷한 이슈가 있다면, 서로 독립적인 평가 사례로 봐도 되는지 살펴야 한다.
...
**관련 코드 찾기.** 이슈에 나온 용어와 API 이름, 관련 테스트, 제공된 stack trace에서 검색을 시작한다. 인덱스에 있는 symbol을 찾았으면 그래프를 따라가 보고, 없으면 소스 검색을 이어 간다. 검색이 유용했는지는 필요한 코드를 찾았고, 그 코드가 올바른 수정에 도움이 되었는지로 판단한다.
...
**테스트와 종료.** 현재 의심하는 원인이 맞는지, 다른 원인 때문인지 가려낼 수 있는 테스트를 실행한다. 코드를 바꾸지 않은 채 이미 통과한 테스트만 반복해도 얻는 것은 적다. 구현과 관련 동작을 확인했다면 정리할 시간을 남겨 두고 `submit_patch()`를 명시적으로 호출한다. harness가 마지막 패치를 회수하는 기능도 있지만, 에이전트가 완료를 선언하면 어디서 작업을 마쳤는지 기록을 읽기 쉽다.
...
주최 측의 `sample_submission`에는 `skip_summarization: true`로 설정한 `AgentTool` 예제가 있다. 이렇게 하면 조사 중에 오간 상세 기록을 주 에이전트의 대화 이력에 모두 넣지 않아도 된다. 다만 모델 호출이 늘고, 두 번째 에이전트 역시 문제를 잘못 이해할 수 있다. 조사에 필요한 도구만 주고 대회에서 지정한 같은 모델을 쓰게 한 뒤, 에이전트 하나로 실행한 결과와 비교하자. 에이전트 수를 더 늘릴지는 그 비교를 보고 정하면 된다.
...
LoRA는 low-rank adaptation의 약자다. 모델에서 선택한 가중치 행렬에 더할 변화량을 적은 수의 parameter로 표현해 학습한다. 행이 $d_{\mathrm{out}}$개, 열이 $d_{\mathrm{in}}$개인 가중치 행렬 $W$를 생각해 보자. 기존 가중치는 고정해 두고, 각 원소를 얼마나 바꿀지 전부 따로 학습하는 대신 한쪽 차원이 작은 두 행렬을 학습한다.
...
위 계산은 가상의 행렬 하나에 필요한 parameter 수를 비교한 것이다. Gemma adapter의 파일 크기나 GPU 메모리 사용량을 추정한 값은 아니다. 실제로 필요한 자원은 적용할 layer와 저장 정밀도에 따라 달라지며, 메타데이터와 activation, optimizer state 등도 고려해야 한다. adapter 파일에는 기본 모델 전체 대신 학습한 변화량을 저장한다. 이 대회에서는 adapter를 써도 되고 쓰지 않아도 된다. 여러 에이전트가 지정된 기본 모델을 공유하면서 각각 다른 adapter를 쓸 수도 있다.
...
무엇을 학습시킬지는 실제 실패 기록을 보고 정해야 한다. **Supervised fine-tuning(SFT)**으로는 코드를 조사하고, 도구를 호출하고, 수정한 뒤 검증하는 좋은 예를 가르칠 수 있다. 강화학습(reinforcement learning, RL)에서는 실제 실행 결과를 보상으로 쓸 수 있지만, 그만큼 실행 기록인 trajectory를 만들고 검증하는 비용이 든다. 짧게 끝난 실행에 무조건 높은 보상을 주면 버그를 고치기 전에 멈추는 쪽으로 학습할 수 있다. 최종 성공 여부만 보더라도, 검증 환경이 제대로 동작하는지 살피지 않으면 잘못된 검증 결과를 학습에 사용할 수 있다.

LoRA와 SFT, RL은 구분해서 이해해야 한다. **LoRA는 parameter의 변화량을 어떻게 표현하고 학습할지**를 정하는 방법이다. **SFT와 RL은 무엇을 보고 배울지**에 관한 방법이다. 따라서 LoRA adapter를 SFT로 학습하는 식으로 함께 사용할 수 있다. [LoRA 원 논문][lora-paper]
...
작은 학습 실험부터 시작하더라도 평가기가 제대로 동작하는지 먼저 확인해야 한다. 어떤 실패를 줄이려는지도 구체적으로 정한다. 예를 들어 검증된 오류 복구 사례를 조금 학습한 뒤, 개발에 쓰지 않은 이슈에서 잘못된 편집 요청을 반복하는 일이 줄었는지 볼 수 있다. 학습이 도움이 되었는지는 최종 해결률과 실행 시간으로 판단한다. **training loss**는 학습 목표에 얼마나 잘 맞췄는지를 나타내는 값이므로, 새 이슈를 해결하는 능력은 별도로 평가해야 한다.

reference patch에는 무엇을 바꿨는지가 남아 있다. 그 파일만으로는 답을 찾아가기까지 어떤 코드를 읽고 어떤 도구를 썼는지 알 수 없다. 학습에 trajectory를 쓰려면 에이전트가 문제를 푸는 과정을 실행하고 기록한 뒤, 최종 패치도 검증해야 한다. 모은 예제가 무엇을 가르치는지 살펴볼 수 있도록 성공한 시도와 실패한 시도를 함께 보관한다.

나라면 baseline이 어떤 문제에서 왜 실패하는지 먼저 살펴본 뒤 adapter의 rank, 학습 데이터, 최적화 방법을 정하겠다. adapter 형식이 맞고 학습이 끝까지 실행되었다는 사실만으로 성능이 좋아졌다고 볼 수는 없다. 개발에 쓰지 않은 문제를 실제로 더 많이 해결하는지 확인해야 한다.
...
그래서 adapter 없는 baseline도 계속 개선하며 남겨 두려고 한다. 추가 학습의 효과를 보려면 처음 만든 프롬프트가 아니라, 학습 없이 개선한 에이전트 중 가장 잘한 버전과 비교해야 한다. 인터페이스와 단순한 에이전트에 관한 연구를 보면 이런 접근을 시도할 이유는 충분하다. 다만 Gemma가 어디까지 좋아질지는 직접 측정해야 한다.
...
이때는 반복해서 틀리는 부분을 학습 예제로 가르쳐 볼 만하다. 학습한 결과를 제출 형식에 맞춰 묶고, 실제 평가에 쓸 환경에서 새로운 이슈를 풀게 해 배운 내용을 활용하는지 확인한다. training loss가 낮아지고 설명이 매끄러워졌거나 adapter가 잘 로드되었다는 것만으로는 부족하다. 정답 위치와 reference patch를 알려 주지 않아도 개선이 유지되어야 한다.
...
새로 푼 문제가 개발 데이터에서 비중이 큰 저장소에 몰려 있거나, 저장소 holdout에서는 오히려 성능이 떨어진다면 주의해서 봐야 한다. 여러 번 실험한 문제와, 설정을 조정하기 전에 따로 남겨 둔 문제에서 해결률 차이가 큰지도 확인한다. 어떤 검사도 비공개 평가를 그대로 대신하지는 못한다. 그래도 이런 결과가 나왔다면 구조를 더 복잡하게 만들기보다, 더 다양한 문제에서 먼저 확인하는 편이 낫다.
...
그때마다 처음부터 다시 시작하지 않으려면, 비교에 쓴 에이전트 버전과 문제 목록, 실행 환경을 남겨 두어야 한다. 기존 비교 결과를 보존한 채 새 공지에서 달라진 조건만 확인해 보면 된다. 나는 처음부터 복잡한 구조를 정하기보다, 지금의 실패가 앞의 세 경우 중 어디에 가까운지 실행 기록으로 알아가는 일이 더 중요하다고 본다.
...
ZIP에 넣을 것은 에이전트 패키지다. Kaggle 평가 시스템은 이를 비공개 문제에서 실행해 패치를 만들고 평가한다. 공개 개발 문제의 수정 내용을 표로 제출하는 방식이 아니다. 이 글의 실습 예제 자체는 아직 제출하거나 채점받지 않았다. 제출 창의 사용법을 확인한 것과 이 예제의 성능을 확인한 것은 별개다. 프로젝트에서 진행한 다른 제출의 점수도 이 예제의 성능으로 볼 수 없다.
...
Paper Track에서는 순위와 별개로 연구 결과를 남길 수 있다. 기존 symbol에서 시작하는 그래프가 언제 코드 탐색에 도움이 되는지, 도구 출력을 어떻게 요약해야 디버깅에 필요한 정보가 남는지, 테스트에 쓸 시간과 수정에 쓸 시간을 어떻게 나눌지 따져 볼 수 있다. 주최 측은 tuning, 코드 이해, 문제와 벤치마크, 그래프 추론도 연구 주제로 제안한다. 질문이 분명하고 다른 사람이 재현할 수 있는 결과라면, 리더보드 최고 점수를 얻지 못했더라도 연구 결과로 정리할 가치가 있다.
...
첫 주에는 아래 순서로 진행해 볼 수 있다. 각 단계에 며칠을 쓸지는 GPU를 사용할 수 있는 시간과 준비 상태에 따라 달라진다. 날짜에 맞춰 진도를 나가기보다, 그 단계에서 확인하려던 내용을 알아낸 뒤 다음으로 넘어가자.
...
이 글을 따라 해 본다면 첫 목표는 실행 과정과 결과를 확인할 수 있는 baseline을 만드는 것이다. 미리 정해 둔 소수의 문제에서 에이전트를 끝까지 실행하고, 문제별 패치와 테스트 로그, 실행 시간을 남긴다. 그다음 한 가지를 바꿔, 그 변경을 설계할 때 쓰지 않은 문제에서도 더 잘 푸는지 확인한다.

첫 실험에서는 이렇게 물으면 된다. **이 환경에서 에이전트는 어느 단계에서 막히는가? 무엇을 바꾸면 같은 제한 시간 안에 더 많은 문제를 풀 수 있을까?** 다음 실험은 그 답을 찾는 데서 시작한다.
