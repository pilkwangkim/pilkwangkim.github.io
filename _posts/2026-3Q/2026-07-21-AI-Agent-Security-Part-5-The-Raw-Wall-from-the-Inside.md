...
*Update ([Part 6]({{ site.baseurl }}/posts/AI-Agent-Security-Part-6-Throughput-Working-Note-Established-Closed-Open/)): two things left local here are now settled on the server. Multipost is confirmed a wash on the served, cache-warm engine — the tool-schema prefix is cached across candidates, so extra posts are pure extra generation, and a live A/B **decreases** in the bundle size, which is the cache-on signature. And "decode is at its floor" (§10, §12) is refined: the floor holds at the instruction level, but the address inside the tool call is a free field the scorer reads only by host, so it compresses to a shorter faithful form worth a few percent of throughput. The §11 note that self-sizing dissolves the local-vs-server cost gap is narrowed there — it resolves the per-candidate cost, but not how close to the replay-budget cliff a given draw allows, which stays the one genuinely server-only unknown.*
