...
6월 30일 기준 local store는 5일치 리플레이 로그를 처리했다. 2026-06-30 batch만으로도 의미 있는 field snapshot을 만들 수 있었다.
...
여기서 중요한 것은 처리량 자체가 아니라 evidence chain의 모양이다. replay는 compact analysis table로 변환하고, 모든 파생 산출물은 source date 기준으로 나누며, benchmark run은 그 panel weight를 만든 field snapshot과 연결해 둔다.

이렇게 해 두면 나중의 주간 리포트에서 "이 덱은 20260630 필드 가중 패널을 이겼기 때문에 선택되었고, 그 weight는 이 리플레이 로그에서 왔다"고 말할 수 있다.
...
| MetaStore / FieldStore buildout | raw replay를 매번 읽는 대신 date-partitioned compact store로 옮겼다. | raw replay는 증거이고, 분석 계층은 compact하고 queryable해야 하며 source date와 연결되어야 한다. | `Archive/MMDD`, `MetaStore/YYYYMMDD`, `FieldStore/YYYYMMDD` |
