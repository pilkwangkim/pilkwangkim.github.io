...
Parts 1–3 moved Kaggle's [AI Agent Security — Multi-Step Tool Attacks](https://www.kaggle.com/competitions/ai-agent-security-multi-step-tool-attacks) benchmark from its replay contract to the v3.1.2 throughput model: public score is a cross-model mean, clean single-post rows are throughput-limited, and shorter wrap-up and framing cues reached the $\approx60$ plateau. This note covers the experiments run **from July 1 through July 15, 2026**. It tests runtime sizing, replay-budget control, model-specific routing, control-token injection, and then checks the multi-post hypothesis on the public model harness. The active question is whether the next gain comes from cheaper candidates or from raising the number of scored posts per candidate.
